...
- **Code Comments:** Write every code comment in English. Beyond consistency, Biome's React Compiler rule crashes on a byte index that lands inside a multibyte character, and the crash aborts the analysis of that file while the command still exits 0 — a Japanese comment silently costs the file its lint coverage.
