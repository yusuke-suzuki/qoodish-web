...
const feedbackWebhook = new IncomingWebhook(
  functions.config().app.feedback_webhook_url
);
const cloudBuildWebhook = new IncomingWebhook(
  functions.config().app.cloud_build_webhook_url
);
...
app.get(pageRoutes, async (req, res) => {
  if (isBot(req)) {
    ...
    res
      .status(200)
      .send(
        ejs.render(fs.readFileSync('./metadata.html').toString(), {
          metadata: metadata,
          fbAppId: functions.config().app.fb_app_id
        })
      );
  } else {
...
exports.generateThumbnail = functions
  ...
  .storage.bucket(functions.config().app.firebase_image_bucket_name)
  .object()
  .onFinalize(object => {
