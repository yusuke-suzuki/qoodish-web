...
import DeleteDeviceFailed from './errors/DeleteDeviceFailed';
...
class QoodishClient {
  ...
  async deleteDevice(token, registrationToken) {
    ...
    if (response.ok) {
      ...
      const json = await response.json();
      throw new DeleteDeviceFailed(response.status, json.detail);
    }
  }
